...
#include "stdafx.h"
#include "DeepLearningPerceptron.h"
...
#define PARTIAL_RUN 0
#define TANH_INPUT 0
#define HIDDEN_NEURONS 100 //120 //100
#define DEEP_NEURONS 100 //60 //100
#define PARTIAL_RUN_MAX_EPOCHS 20
#define PARTIAL_RUN_SAMPLES 60000
#define MAX_EPOCHS 200 //77
...
int
check(int label, real *results, real *answers)
{
	real max = 0.0;
	int max_idx = 0;
	int right = 0;
	...
	/*
	int right = 1; real threshold = 0.5;
	for (int j = 0; j < 10; j++) {
		if (label == j) {
			answers[j] = 1.0;
...
run_nn2(void)
{
	...
	int topology[] = { n, DEEP_NEURONS, HIDDEN_NEURONS, 10 };
	//int topology[] = { n, 24 * 24, 12 * 12, 8 * 8, 4 * 4, 10 };
	real *inputs = (real *)malloc(n * sizeof(real));

	net = mlp_net_new(topology, sizeof(topology) / sizeof(topology[0]));
...
//	UpdateWindow(hDialog);
	...
	int total = 0;
#if PARTIAL_RUN == 1
	int max_epochs = PARTIAL_RUN_MAX_EPOCHS; // 3;
#else
...
#if PARTIAL_RUN == 1
		int n_training = PARTIAL_RUN_SAMPLES; // mnist_training->items;
#else
	...
	for (i = 0; i < n_training; i++) {
		if (i % 1000 == 0) {
			...
			for (j = 0; j < n; j++) {
#if TANH_INPUT == 0
				inputs[j] = 1.0 / 255.0 * (real)mnist_training->images[indexes[i]][j];
#else
				inputs[j] = ((2.0 / 255.0) * (real)mnist_training->images[indexes[i]][j]) - 1.0;
#endif
			}



			for (int x = 0; x < 1; x++) { // repeat training multiple times an per image
				mlp_net_feedforward(net, inputs, results);

				int right = check(mnist_training->labels[indexes[i]], results, answers);

				if (x == 0)
					if (right)
						correct++;

				mlp_net_train(net, answers);
		...
		for (i = 0; i < n_testing; i++) {
			...
			for (j = 0; j < n; j++) {
#if TANH_INPUT == 0
				inputs[j] = 1.0 / 255.0 * (real)mnist_testing->images[i][j];
#else
				inputs[j] = ((2.0 / 255.0) * (real)mnist_testing->images[i][j]) + 1.0;
#endif
			}
			...
			for (int x = 0; x < 1; x++) {
				...
				int right = check(mnist_testing->labels[i], results, answers);;
	...
	/*
	 * print network layers to debug window
	 */
	for (int i = 1; i < net->n_layers; i++) {
		...
		for (int j = 0; j < net->layers[i]->n_perceptrons; j++) {
			sprintf_s(buf, BUFSIZ, " %d (%d) [ ", j, net->layers[i]->perceptrons[j]->n_inputs);
			str = char_to_wchar(buf);
			OutputDebugString(str);
			free(str);

			for (int k = 0; k < net->layers[i]->perceptrons[j]->n_inputs; k++) {
				float weight = net->layers[i]->perceptrons[j]->weights[k];

				if (net->layers[i]->perceptrons[j]->inputs[k]->type == PERCEPTRON_BIAS)
...
Paint(HDC hdcMem)
{
	...
	for (int i = 0; i < net->layers[1]->n_perceptrons; i++) {
		perceptron_t *p = net->layers[1]->perceptrons[i];
	...
	for (int i = 0; i < net->layers[2]->n_perceptrons; i++) {
		perceptron_t *p = net->layers[2]->perceptrons[i];

		if (p->type == PERCEPTRON_BIAS)
			continue;

		for (int x = 0; x < p->n_inputs / mnist_testing->cols; x++) {
			for (int y = 0; y < mnist_testing->cols; y++) {
				uint8_t pixel = (uint8_t)(sigmoid(p->weights[(x * mnist_testing->cols) + y]) * 255);
				color = RGB(255 - pixel, 255 - pixel, 255 - pixel);
				SetPixel(hdcMem, (i * mnist_testing->rows) + x, y + mnist_testing->rows, color);
			}
		}
	}
...
#if 1
	real inputs[10];
	real *outputs = (real *)malloc(mnist_testing->rows * mnist_testing->cols * sizeof(real));

	for (int j = 0; j < 10; j++) {
		// try to get feedback on the number 4
		for (int i = 0; i < 10; i++)
			inputs[i] = 0;
		inputs[j] = 1.0;

		mlp_net_feedback(net, inputs, outputs);

		for (int i = 0; i < mnist_testing->rows * mnist_testing->cols; i++) {
			int y = i / mnist_testing->cols;
			int x = i % mnist_testing->cols;
			/*
			real min = 0, max = 0;
			for (int k = 0; k < mnist_testing->rows * mnist_testing->cols; k++) {
				if (outputs[i] < min)
					min = outputs[i];
			...
			*/
			//uint8_t pixel = (uint8_t)(outputs[i] * 255);
			//uint8_t pixel = (uint8_t)((real)outputs[i] / (real)(max - min) * (real)255.0);
			uint8_t pixel = (uint8_t)(sigmoid(outputs[i]) * 255.0);
			color = RGB(255 - pixel, 255 - pixel, 255 - pixel);
			SetPixel(hdcMem, (j * mnist_testing->cols) + x, (mnist_testing->rows * 3) - y, color);
		}
	}
	//free(outputs);
